...
describe("DefaultFetcher", () => {
  ...
  it("should throw if response is not ok", async () => {
    ...
    await expect(fetcher.fetch("https://example.com/missing.json")).rejects.toThrow(
      "Failed to fetch from https://example.com/missing.json: 404 Not Found",
    );
  });
  ...
  it("should throw if response is not a JSON array", async () => {
    ...
    await expect(fetcher.fetch("https://example.com/bad.json")).rejects.toThrow(
      "Source did not return a JSON array (got object)",
    );
  });
  ...
  it("should throw a timeout error when request exceeds timeout", async () => {
    ...
    await expect(fastFetcher.fetch("https://example.com/slow")).rejects.toThrow(
      "Request to https://example.com/slow timed out after 1ms",
    );
  });
  ...
  it("should re-throw non-abort errors as-is", async () => {
    ...
    await expect(fetcher.fetch("https://example.com/fail")).rejects.toThrow("DNS failure");
  });
});
